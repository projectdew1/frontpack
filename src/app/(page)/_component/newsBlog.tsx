        ...
        <div className=" max-w-screen-xl">
                ...
                <div className=" relative h-[250px] overflow-hidden rounded-2xl">
                  {news.localImage ? (
                    <Image
                      ...
                      className=" object-cover rounded-xl select-none transition duration-300 group-hover:scale-110" // just an example
                    />
                  ) : null}
                </div>
          ...
          <a className="flex flex-row justify-center cursor-pointer mt-10 ">
            {pageTotal == page ? null : (
              <div
                onClick={nextPage}
                className="text-md font-extralight p-2 px-10 text-center rounded-full border border-kmsppad text-kmsppad "
              >
                {"เพิ่มเติม"}
              </div>
            )}
